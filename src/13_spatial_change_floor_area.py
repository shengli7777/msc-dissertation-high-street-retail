...
FLOOR_CSV = OUT / "grid_250m_floor_area_change_2016_2025.csv"
...
CLASSES = [
    ("Decrease > 500 sqm", "#D73027"),
    ("Decrease 1-500 sqm", "#FC8D59"),
    ("No change", "#E6E6E6"),
    ("Increase 1-500 sqm", "#91BFDB"),
    ("Increase > 500 sqm", "#4575B4"),
]
CLASS_COLOR = dict(CLASSES)
...
def draw_cells(draw, cells, tr):
    ordered = pd.Categorical(
        cells["floor_area_change_class"],
        categories=[name for name, _ in CLASSES],
        ordered=True,
    )
    cells = cells.assign(_order=ordered).sort_values("_order")
    for row in cells.itertuples():
        color = CLASS_COLOR.get(row.floor_area_change_class, "#CCCCCC")
...
def draw_legend(draw, x, y):
    text(draw, (x, y), "Floor-area change", size=26, bold=True)
...
def main():
    ...
    for city, panel in panels.items():
        ...
        draw_legend(draw, panel[0] + panel[2] - 420, panel[1] + panel[3] - 390)
    save_all(img, "figure_spatial_change_retail_floor_area_vertical")
    ...
    for city in CITY_ORDER:
        ...
        draw_legend(single_draw, single_panel[0] + single_panel[2] - 420, single_panel[1] + single_panel[3] - 385)
        save_all(single_img, f"figure_spatial_change_retail_floor_area_{city.lower()}")
