...
BINS = [
    (0, 0, "0%", "#F7F2EE"),
    (0, 0.05, ">0-5%", "#F2D8CA"),
    (0.05, 0.10, "5-10%", "#E9B79E"),
    (0.10, 0.20, "10-20%", "#D88467"),
    (0.20, 0.50, "20-50%", "#B9554F"),
    (0.50, math.inf, ">50%", "#7E2F3B"),
]
...
def vacancy_color(value, known_count):
    if not known_count or pd.isna(known_count) or known_count <= 0 or pd.isna(value):
        return PAL["no_known"]
    value = float(value)
    if value == 0:
        return BINS[0][3]
    for low, high, _, color in BINS[1:]:
        if low < value <= high:
            return color
    return BINS[-1][3]
...
def draw_grid_cells(draw, grid, city, year, tr):
    cells = grid[grid["city"].eq(city) & grid["year"].eq(year)].sort_values("vacancy_rate", na_position="first")
    for row in cells.itertuples():
        x1, y1 = tr(row.grid_x, row.grid_y + GRID_SIZE)
        x2, y2 = tr(row.grid_x + GRID_SIZE, row.grid_y)
        color = vacancy_color(row.vacancy_rate, row.known_occupation_count)
        rect(draw, (x1, y1, x2, y2), color, outline=None)
...
def draw_legend(draw, x, y):
    text(draw, (x, y), "Known-case vacancy rate", size=18, bold=True)
    ...
    yy = y + 256
    rect(draw, (x, yy - 11, x + 24, yy + 11), PAL["no_known"], outline="#FFFFFF", width=1)
    text(draw, (x + 34, yy - 12), "No known status", size=15)
    yy += 45
...
def main():
    grid = pd.read_csv(OUT / "grid_250m_vacancy_selected_years.csv")
    ...
    save_all(img, "figure_vacancy_indicator_map_2016_2025")
